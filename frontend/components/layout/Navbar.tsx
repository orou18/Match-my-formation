  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between gap-3">
        {/* LOGO */}
        <Link href={`/${locale}`} className="relative h-12 w-48">
          <Image
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {active && (
                  <div className="absolute inset-0 bg-white shadow-sm rounded-full z-0" />
                )}
              </Link>
            );
          })}
      ...
      {/* MOBILE MENU OVERLAY */}
      {open && (
        <div className="fixed inset-x-0 top-full bg-white z-40 overflow-hidden flex flex-col md:hidden border-t border-gray-100 shadow-2xl">
            ...
            {navItems.map((item, i) => (
              <div key={item.label}>
                <Link
                  ...
                  className={`text-3xl font-black ${pathname === item.href ? "text-primary" : "text-secondary"}`}
                >
                  {item.label}
                </Link>
              </div>
