...
export default function NotificationItem({ 
  ...
  return (
    ...
    >
      ...
      {/* Icône stylisée */}
      <div className={`w-12 h-12 ${config.light} rounded-2xl flex items-center justify-center ${config.text} shrink-0 transition-transform group-hover:scale-110`}>
        <Icon size={22} />
      </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex justify-between items-start gap-2">
          <h4 className={`font-bold text-sm md:text-base leading-tight ${isRead ? "text-gray-500" : "text-[#002B24]"}`}>
            {title}
          </h4>
