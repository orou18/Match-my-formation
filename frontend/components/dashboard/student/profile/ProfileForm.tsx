...
export default function ProfileForm() {
  const inputStyle = "w-full bg-white border border-gray-200 rounded-2xl px-6 py-4 text-sm font-medium focus:ring-2 focus:ring-primary/10 focus:border-primary outline-none transition-all";
  
  return (
    <div className="bg-white rounded-[2.5rem] p-8 md:p-12 shadow-sm border border-gray-50">
      <h2 className="text-2xl font-bold text-[#002B24] mb-10">Informations personnelles</h2>

      <div className="mb-12">
        <p className="text-xs font-black uppercase tracking-widest text-gray-400 mb-6">Photo de profil</p>
        <div className="flex items-center gap-8">
          ...
          <div className="space-y-3">
            ...
            <p className="text-[11px] text-gray-400 font-medium ml-1">JPG, PNG ou GIF. Max 2MB.</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
        {[
          ...
          <div key={f.label} className="space-y-3">
            <label className="text-xs font-black text-[#002B24] ml-1 uppercase tracking-tighter opacity-60">{f.label}</label>
            <input type={f.type} defaultValue={f.value} className={inputStyle} />
          </div>
        ))}

        {["Pays", "Langue préférée"].map((label) => (
          <div key={label} className="space-y-3 relative">
            <label className="text-xs font-black text-[#002B24] ml-1 uppercase tracking-tighter opacity-60">{label}</label>
            <div className="relative">
              <select className={`${inputStyle} appearance-none cursor-pointer`}>
                <option>{label === "Pays" ? "France" : "Français"}</option>
                <option>{label === "Pays" ? "Côte d'Ivoire" : "Anglais"}</option>
              </select>
