    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <motion.div
          ...
          className="bg-white rounded-xl p-6 shadow-sm border border-gray-100"
        >
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Performance des Parcours</h3>
          <div className="space-y-4">
            {analytics.pathwayPerformance.map((pathway, index) => (
              <div key={pathway.pathwayName} className="border-b border-gray-100 pb-4 last:border-0">
                <div className="flex items-center justify-between mb-2">
                  <h4 className="font-medium text-gray-900">{pathway.pathwayName}</h4>
                  <span className="text-sm text-gray-600">{pathway.enrolledEmployees} employés</span>
                </div>
                <div className="space-y-2">
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-600">Progression moyenne</span>
                    <span className="font-medium">{pathway.averageProgress}%</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-medium">{pathway.completionRate}%</span>
                  </div>
                </div>
              </div>
            ))}
      ...
      <motion.div
        ...
        className="bg-white rounded-xl p-6 shadow-sm border border-gray-100"
      >
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Progression des Employés</h3>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200">
                ...
                <th className="text-left py-3 px-4 text-sm font-medium text-gray-700">Dernière activité</th>
              </tr>
            </thead>
            <tbody>
              {analytics.employeeProgress.map((employee, index) => (
                <tr key={index} className="border-b border-gray-100">
                  ...
                  <td className="py-3 px-4 text-sm text-gray-600">{employee.lastActivity}</td>
                </tr>
              ))}
            </tbody>
          </table>
