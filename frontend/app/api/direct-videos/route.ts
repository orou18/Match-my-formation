...
// Base de données directe pour les vidéos
...
export async function GET(request: NextRequest) {
  try {
    console.log('DIRECT VIDEOS - Récupération des vidéos publiques');
    
    // Filtrer uniquement les vidéos publiques et publiées
    const publicVideos = directVideos.filter(video => 
      video.visibility === 'public' && video.is_published === true
    );
    
    console.log('DIRECT VIDEOS - Vidéos trouvées:', publicVideos.length);
    console.log('DIRECT VIDEOS - Titres:', publicVideos.map(v => v.title));
    ...
    console.error('DIRECT VIDEOS - Erreur:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'add') {
      console.log('DIRECT VIDEOS - Ajout vidéo:', video.title);
      
      // Ajouter la nouvelle vidéo
      directVideos.push(video);
      
      console.log('DIRECT VIDEOS - Total vidéos après ajout:', directVideos.length);
    ...
    console.error('DIRECT VIDEOS - Erreur POST:', error);
    return NextResponse.json({ error: 'Erreur serveur' }, { status: 500 });
  }
}
