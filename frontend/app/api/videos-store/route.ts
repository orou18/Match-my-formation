  ...
  try {
    ...
    switch (action) {
      ...
      case 'update':
        ...
        const updatedVideo = await videosStore.updateVideo(parseInt(videoId), updates);
        if (!updatedVideo) {
          return NextResponse.json({ 
      ...
      case 'publish':
        ...
        const publishedVideo = await videosStore.publishVideo(parseInt(videoId));
        if (!publishedVideo) {
          return NextResponse.json({ 
            error: "Vidéo non trouvée" 
          }, { status: 404 });
      ...
      case 'unpublish':
        ...
        const unpublishedVideo = await videosStore.unpublishVideo(parseInt(videoId));
        if (!unpublishedVideo) {
          return NextResponse.json({ 
            error: "Vidéo non trouvée" 
          }, { status: 404 });
