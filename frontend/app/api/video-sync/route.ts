...
// Base de données pour la synchronisation
let syncVideos: any[] = [
  {
    id: '1',
...
export async function GET(request: NextRequest) {
  try {
    console.log('VIDEO SYNC - Récupération des vidéos publiques');
    
    // Filtrer uniquement les vidéos publiques et publiées
    const publicVideos = syncVideos.filter(video => 
      video.visibility === 'public' && video.is_published === true
    );
    
    console.log('VIDEO SYNC - Vidéos trouvées:', publicVideos.length);
    console.log('VIDEO SYNC - Titres:', publicVideos.map(v => v.title));
    ...
    console.error('VIDEO SYNC - Erreur:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'add') {
      console.log('VIDEO SYNC - Ajout vidéo:', video.title);
      
      // Ajouter la nouvelle vidéo
      syncVideos.push(video);
      
      console.log('VIDEO SYNC - Total vidéos après ajout:', syncVideos.length);
    ...
    console.error('VIDEO SYNC - Erreur POST:', error);
    return NextResponse.json({ error: 'Erreur serveur' }, { status: 500 });
  }
}
