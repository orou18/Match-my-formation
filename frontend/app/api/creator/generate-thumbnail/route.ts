...
export async function POST(request: NextRequest) {
  try {
    ...
    const tempVideoPath = join(process.cwd(), 'temp', `${Date.now()}-${videoFile.name}`);
    ...
    try {
      ...
      return NextResponse.json({
        message: 'Utilisation de la miniature par défaut (FFmpeg non disponible)',
    ...
    return NextResponse.json({ error: 'Erreur lors de la génération de la miniature' }, { status: 500 });
  }
}
