...
class EmployeeStorage {
  private static readonly STORAGE_KEY = "employees.json";

  // Obtenir tous les employés
  static getEmployees(): Employee[] {
    try {
      return readJsonStore(this.STORAGE_KEY, this.getDefaultEmployees());
    } catch (error) {
  ...
  private static saveEmployees(employees: Employee[]): void {
    try {
      writeJsonStore(this.STORAGE_KEY, employees);
    } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      ...
      error.message.includes("email est déjà utilisé")
    ) {
      return NextResponse.json(
