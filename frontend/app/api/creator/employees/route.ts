...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingEmployee = employeesStore.find(emp => emp.email === payload.email);
    if (existingEmployee) {
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const employeeIndex = employeesStore.findIndex(emp => emp.id === parseInt(id));
    if (employeeIndex === -1) {
      return NextResponse.json(
        {
          success: false,
    ...
    if (updateData.email) {
      const existingEmployee = employeesStore.find(emp => emp.email === updateData.email && emp.id !== parseInt(id));
      if (existingEmployee) {
        return NextResponse.json(
          {
            success: false,
