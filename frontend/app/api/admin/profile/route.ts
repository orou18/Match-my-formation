...
export async function GET(request: NextRequest) {
  try {
    ...
    const profile = mockAdminProfiles[userEmail as keyof typeof mockAdminProfiles];
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const hasEditPermission = profile.permissions.includes('profile_edit') || isSuperAdmin;
    ...
    const allowedFields = isSuperAdmin 
      ? ['name', 'email', 'bio', 'phone', 'location', 'department', 'emailNotifications', 'language']
    ...
    console.log('ADMIN PROFILE - Profil mis à jour pour:', userEmail, filteredData);
