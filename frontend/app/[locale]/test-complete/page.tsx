...
export default function TestCompletePage() {
  ...
  const runCompleteTest = async () => {
    ...
    setTestResults((prev) => [
      ...prev,
      `   Authentifié: ${initialAuth ? "OUI" : "NON"}`,
    ]);
    ...
    try {
      ...
      if (registerResponse.ok) {
        ...
        setTestResults((prev) => [
          ...prev,
          `   ✅ Inscription réussie: ${registerData.user.name}`,
        ]);
        setTestResults((prev) => [
          ...prev,
        ...
        setTestResults((prev) => [
          ...prev,
          `   🔐 Token généré: ${registerData.token.substring(0, 20)}...`,
        ]);
      } else {
        const errorData = await registerResponse.json();
        setTestResults((prev) => [
          ...prev,
          `   ❌ Erreur inscription: ${errorData.message}`,
        ]);
      }
    } catch (error) {
    ...
    try {
      ...
      if (loginResponse.ok) {
        ...
        setTestResults((prev) => [
          ...prev,
          `   ✅ Connexion réussie: ${loginData.user.name}`,
        ]);
        setTestResults((prev) => [
          ...prev,
        ...
        setTestResults((prev) => [
          ...prev,
          `   ❌ Erreur connexion: ${errorData.message}`,
        ]);
      }
    } catch (error) {
      setTestResults((prev) => [...prev, `   ❌ Erreur réseau: ${error}`]);
    ...
    setTestResults((prev) => [
      ...prev,
      `   Authentifié: ${afterLogout ? "OUI ❌" : "NON ✅"}`,
    ]);
    ...
    setTestResults((prev) => [
      ...prev,
      `   ✅ Page accessible sans redirection`,
    ]);
          ...
          <div className="space-y-1">
            {testResults.length === 0 ? (
              <p className="text-gray-500">
                Cliquez sur "Exécuter les tests complets" pour commencer
              </p>
            ) : (
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <button
                onClick={() =>
                  testPageAccess("/register", "Page d'inscription")
                }
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <button
                onClick={() =>
                  testPageAccess("/dashboard/student", "Dashboard Student")
                }
              ...
              <button
                onClick={() =>
                  testPageAccess("/dashboard/creator", "Dashboard Creator")
                }
              ...
              <button
                onClick={() =>
                  testPageAccess("/dashboard/admin", "Dashboard Admin")
                }
