...
export default function AdminCreators() {
  ...
  const [newCreator, setNewCreator] = useState({ name: '', email: '', category: 'Marketing', bio: '', expertise: '' });
  ...
  const handleCreateCreator = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setNewCreator({ name: '', email: '', category: 'Marketing', bio: '', expertise: '' });
  ...
  const handleUpdateCreator = async (creatorId: string, updates: any) => {
    try {
      ...
      if (response.ok) {
        ...
        setCreators(creators.map(creator => creator.id === creatorId ? updatedCreator : creator));
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Gestion Créateurs</h1>
          <p className="text-gray-600 mt-1">{filteredCreators.length} créateurs trouvés</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        {[
          { label: "Total Créateurs", value: creators.length, icon: Users, color: "blue", change: "+12%" },
          { label: "Revenus Totaux", value: formatCurrency(creators.reduce((sum, c) => sum + c.revenue, 0)), icon: DollarSign, color: "green", change: "+23%" },
          { label: "Étudiants Totaux", value: formatNumber(creators.reduce((sum, c) => sum + c.students, 0)), icon: Users, color: "purple", change: "+18%" },
          { label: "Vues Totales", value: formatNumber(creators.reduce((sum, c) => sum + c.totalViews, 0)), icon: Play, color: "orange", change: "+31%" },
        ].map((stat, index) => (
          <motion.div
            key={stat.label}
          ...
          >
            <div className="p-6">
              <div className="flex items-start justify-between mb-4">
                ...
                <span className={`px-2 py-1 rounded-full text-xs font-bold ${getStatusBadge(creator.status)}`}>
                  {creator.status}
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div className="text-center p-3 bg-gray-50 rounded-lg">
                  ...
                  <p className="text-lg font-bold text-gray-900">{creator.courses}</p>
                  <p className="text-xs text-gray-500">Cours</p>
                </div>
                <div className="text-center p-3 bg-gray-50 rounded-lg">
                  <Users size={16} className="mx-auto text-gray-600 mb-1" />
                  <p className="text-lg font-bold text-gray-900">{formatNumber(creator.students)}</p>
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="text-right">
                  <p className="text-lg font-bold text-gray-900">{formatCurrency(creator.revenue)}</p>
