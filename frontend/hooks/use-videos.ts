...
import { videosService, Video, VideoFormData } from '@/lib/services/videos-service';
...
export function useVideos(options: UseVideosOptions = {}) {
  ...
  const updateVideo = useCallback(async (id: number, videoData: Partial<VideoFormData>) => {
    setLoading(true);
    setError(null);
  ...
  const searchVideos = useCallback(async (query: string, searchFilters?: {
    category?: string;
    creator?: string;
    tags?: string[];
  }) => {
...
export function useCreatorVideos(creatorId?: number) {
  ...
  const fetchCreatorVideos = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to fetch creator videos');
      setVideos([]);
    } finally {
      setLoading(false);
    }
...
export function usePublicVideos() {
  ...
  const fetchPublicVideos = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to fetch public videos');
      setVideos([]);
    } finally {
      setLoading(false);
    }
